...
define( 'CUSTOM_METABOXES_DIR', plugins_url() . '/socrata-apps/metaboxes' ); //Change this URL path. Also change the function prefix.
 ...
 */
function socrata_apps_custom_meta_box_field( $field, $meta = null, $repeatable = null ) {
		...
		case 'repeatable':
			...
			foreach( $meta as $row ) {
				...
				foreach ( $repeatable_fields as $repeatable_field ) {
					...
					echo socrata_apps_custom_meta_box_field( $repeatable_field, $meta[$i][$repeatable_field['id']], array( $id, $i ) );
					echo '</p>';
				} // end each field
 ...
 */
function socrata_apps_meta_box_find_field_type( $needle, $haystack ) {
	foreach ( $haystack as $h )
		if ( isset( $h['type'] ) && $h['type'] == 'repeatable' )
			return socrata_apps_meta_box_find_field_type( $needle, $h['repeatable_fields'] );
 ...
 * @return	bool				whether or not the type is in the provided array
 */
function socrata_apps_meta_box_find_repeatable( $needle = 'repeatable', $haystack ) {
	foreach ( $haystack as $h )
 ...
 */
function socrata_apps_meta_box_santitize_boolean( $string ) {
 ...
 */
function socrata_apps_meta_box_sanitize( $string, $function = 'sanitize_text_field' ) {
	switch ( $function ) {
		case 'intval':
 ...
 */
function socrata_apps_meta_box_array_map_r( $func, $meta, $sanitizer ) {
	...
	foreach( $meta as $key => $array ) {
		...
		foreach( $array as $arrayKey => $arrayValue )
			if ( is_array( $arrayValue ) )
				$array[$arrayKey] = socrata_apps_meta_box_array_map_r( $func, $arrayValue, $newSanitizer[$arrayKey] );
 ...
 */
class socrata_apps_custom_add_meta_box {
	...
	function admin_enqueue_scripts() {
		...
		if ( in_array( $pagenow, array( 'post-new.php', 'post.php' ) ) && in_array( get_post_type(), $this->page ) ) {
			// js
			$deps = array( 'jquery' );
			if ( socrata_apps_meta_box_find_field_type( 'date', $this->fields ) )
				$deps[] = 'jquery-ui-datepicker';
			if ( socrata_apps_meta_box_find_field_type( 'slider', $this->fields ) )
				$deps[] = 'jquery-ui-slider';
			if ( socrata_apps_meta_box_find_field_type( 'color', $this->fields ) )
				$deps[] = 'farbtastic';
			if ( in_array( true, array(
				socrata_apps_meta_box_find_field_type( 'chosen', $this->fields ),
				socrata_apps_meta_box_find_field_type( 'post_chosen', $this->fields )
			) ) ) {
			...
			if ( in_array( true, array( 
				socrata_apps_meta_box_find_field_type( 'date', $this->fields ), 
				socrata_apps_meta_box_find_field_type( 'slider', $this->fields ),
				socrata_apps_meta_box_find_field_type( 'color', $this->fields ),
				socrata_apps_meta_box_find_field_type( 'chosen', $this->fields ),
				socrata_apps_meta_box_find_field_type( 'post_chosen', $this->fields ),
				socrata_apps_meta_box_find_repeatable( 'repeatable', $this->fields ),
				socrata_apps_meta_box_find_field_type( 'image', $this->fields ),
				socrata_apps_meta_box_find_field_type( 'file', $this->fields )
			) ) )
			...
			if ( socrata_apps_meta_box_find_field_type( 'date', $this->fields ) || socrata_apps_meta_box_find_field_type( 'slider', $this->fields ) )
				$deps[] = 'jqueryui';
			if ( socrata_apps_meta_box_find_field_type( 'color', $this->fields ) )
				$deps[] = 'farbtastic';
	 ...
	 */
	function admin_head() {
		if ( in_array( get_post_type(), $this->page ) && ( socrata_apps_meta_box_find_field_type( 'date', $this->fields ) || socrata_apps_meta_box_find_field_type( 'slider', $this->fields ) ) ) {
		...
		foreach ( $this->fields as $field) {
			if ( $field['type'] == 'section' ) {
				...
				echo '<tr>
						...
						echo socrata_apps_custom_meta_box_field( $field, $meta );
		...
		foreach ( $this->fields as $field ) {
			...
			if( in_array( $field['type'], array( 'tax_select', 'tax_checkboxes' ) ) ) {
				...
				if ( isset( $new ) && '' == $new && $old ) {
					...
					if ( is_array( $new ) )
						$new = socrata_apps_meta_box_array_map_r( 'socrata_apps_meta_box_sanitize', $new, $sanitizer );
					else
						$new = socrata_apps_meta_box_sanitize( $new, $sanitizer );
