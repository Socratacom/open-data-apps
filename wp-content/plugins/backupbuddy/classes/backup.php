...
class pb_backupbuddy_backup {
	 ...
	 */
	function start_backup_process( $profile, $trigger = 'manual', $pre_backup = array(), $post_backup = array(), $schedule_title = '', $serial_override = '', $export_plugins = array(), $deployDirection = '' ) {
		...
		//if ( 'pull' != $deployDirection ) {
			if ( $this->pre_backup( $serial, $profile, $trigger, $pre_backup, $post_backup, $schedule_title, $export_plugins, $deployDirection ) === false ) {
	 ...
	 *	@return		boolean							True on success; false otherwise.
	 */
	function pre_backup( $serial, $profile, $trigger, $pre_backup = array(), $post_backup = array(), $schedule_title = '', $export_plugins = array(), $deployDirection ) {
		...
		} elseif ( $type == 'deployPull' ) {
			pb_backupbuddy::status( 'message', __( 'Deploy PULL mode.', 'it-l10n-backupbuddy' ) );
		} else {
		...
		*/
		...
		$this->_backup = array(
			...
			'deployment_log'		=>		'',												// URL to the importtbuddy status log for deployments.
		...
		} elseif ( $this->_backup['type'] == 'deployPull' ) {
			$this->_backup['temp_directory'] = backupbuddy_core::getTempDirectory() . $serial . '/';
			$this->_backup['backup_root'] = $this->_backup['temp_directory'];
		} else {
		...
		if ( ( '' == $deployDirection ) && ( '1' != $profile['skip_database_dump'] ) && ( $profile['type'] != 'files' ) ) { // Backup database if not skipping AND not a files only backup.
	...
	function process_backup( $serial, $trigger = 'manual' ) {
		...
		if ( $response === false ) { // Function finished but reported failure.
			...
			if ( pb_backupbuddy::$options['log_level'] == '3' ) { // Full logging enabled.
				pb_backupbuddy::status( 'backupState', json_encode( $this->_backup ) ); //base64_encode( json_encode( $this->_backup ) ) );
			}
	...
	function backup_create_database_dump( $tables, $rows_start = 0 ) {
		...
		global $wpdb;
		pb_backupbuddy::$classes['mysqlbuddy'] = new pb_backupbuddy_mysqlbuddy( DB_HOST, DB_NAME, DB_USER, DB_PASSWORD, $wpdb->prefix, $force_methods, backupbuddy_core::adjustedMaxExecutionTime() ); // $database_host, $database_name, $database_user, $database_pass, $old_prefix, $force_method = array()
	...
	function post_backup( $fail_mode = false, $cancel_backup = false ) {
		...
		if ( true === $cancel_backup ) {
			...
			if ( true === $fail_mode ) {
				...
				if ( $this->_backup['trigger'] != 'deployment' ) {
	...
	public function deploy_sendFile( $state, $sendFile, $sendPath, $sendType, $nextStep, $delete_after = false ) {
		...
		if ( false === backupbuddy_core::send_remote_destination( $state['destination_id'], $sendFile, 'Deployment', $send_importbuddy = false, $delete_after, $identifier = $this->_backup['serial'] . '_' . md5( $sendFile . $sendType ), $destination_settings ) ) {
			$sendFile = ''; // Since failed just sent file to blank so we can proceed to next without waiting.
		}
	...
	public function deploy_push_start( $state ) {
		...
		$this->deploy_sendFile( $state, $this->_backup['archive_file'], '', 'backup', $nextStep, $delete_after = true );
	...
	public function deploy_pull_start( $state ) {
		...
		$deploy = new backupbuddy_deploy( $state['apiKey'], $state );
		
		pb_backupbuddy::status( 'details', 'About to remote call.' );
		if ( false === ( $response = backupbuddy_remote_api::remoteCall( $state['destination'], 'runBackup', array( 'profile' => $this->_backup['profile'] ), $state['minimumExecutionTime'] ) ) ) {
		...
		// Send backup zip. It will schedule its next chunks as needed. When done it calls the nextstep.
		$nextStep = array(
			'function'		=>	'deploy_pull_remoteBackup',
			'args'			=>	array( $state ),
			'start_time'	=>	0,
			'finish_time'	=>	0,
			'attempts'		=>	0,
	...
	public function deploy_push_sendContent( $state ) {
		...
		if ( true !== $state['sendMedia'] ) {
			pb_backupbuddy::status( 'details', 'Skipping send of media files.' );
		} else {
			if ( $mediaFileCount > 0 ) { // Media files remain to send.
				...
				$this->deploy_sendFile( $state, $wp_upload_dir['basedir'] . '/' . $sendFile, $sendFile, 'media', $nextStep );
				return true;
			}
		}
		
		if ( true !== $state['sendPlugins'] ) {
			pb_backupbuddy::status( 'details', 'Skipping send of plugin files.' );
		} else {
			if ( $pluginFileCount > 0 ) { // Plugin files remain to send.
				...
				$nextStep['args'] = array( $state );
				$this->deploy_sendFile( $state, $pluginPath . $sendFile, $sendFile, 'plugin', $nextStep );
				return true;
			}
		}
		
		if ( true !== $state['sendTheme'] ) {
			pb_backupbuddy::status( 'details', 'Skipping send of theme files.' );
		} else {
			if ( $themeFileCount > 0 ) { // Plugin files remain to send.
				...
				$nextStep['args'] = array( $state );
				$this->deploy_sendFile( $state, $themePath . $sendFile, $sendFile, 'theme', $nextStep );
				return true;
			}
		}
	...
	public function deploy_push_renderImportBuddy( $state ) {
		...
		$importbuddyURL = $importbuddyURLRoot . '?ajax=2&v=' . $importbuddyPassword . '&deploy=true&file=' . basename( $this->_backup['archive_file'] );
		...
		// Calculate undo URL.
		$undoDeployURL = $state['destination']['siteurl'] . '/backupbuddy_deploy_undo-' . $response['importFileSerial'] . '.php';
